...
public:
    ...
    int ReverseUpdate(const uint8_t* arr) {
        memset(prev, 0, n_samples);
        memset(n_queue, 0, sizeof(uint32_t)*n_symbols);

        // Restore + update PPA
        // arr[i] is guaranteed to 0 or 1.
        ...
        // Debug
        // for (int i = 0; i < n_samples; ++i) {
        //     const uint8_t ref = arr[ppa[i]*stride] & (~1);
        //     // std::cerr << i << "/" << n_samples << ": " << std::bitset<8>(ref) << " " << std::bitset<8>(TWK_BCF_GT_PACK[prev[i]] << 1) << " Raw=" << std::bitset<8>(prev[i]) << std::endl;
        //     assert((TWK_BCF_GT_PACK[prev[i]] << 1) == ref);
        // }
