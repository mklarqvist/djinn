...
struct djinn_variant_t {
    ...
    // Silly function to add trailing "\t\n" for checksum comparisons
    // against bcftools query -f "[%GT\t]\n".
    ...
    int ToVcf(char* out, const char phasing = '|') const {
        // std::cerr << "unpacked=" << (int)unpacked << std::endl;
        ...
        if (unpacked == DJN_UN_IND) {
            ...
            if (n_allele < 10) {
                ...
                std::cerr << "returning: " << len_vcf << "b" << std::endl;
                return len_vcf;
            } else {
                std::cerr << "not implemented" << std::endl;
                return -1;
