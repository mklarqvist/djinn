...
* under the License.
*/
#ifndef DJINN_EXAMPLE_ITERATE_VCF_H_
#define DJINN_EXAMPLE_ITERATE_VCF_H_
 ...
 */
int IterateVcf(std::string input_file, int model) {
    ...
    while (true) {
        ...
        for (int i = 0; i < djn_decode->n_variants; ++i, ++n_lines) {
            ...
            len_vcf = variant->ToVcf(vcf_out_buffer);
            assert(len_vcf > 0);
